(function() {
  ...
  module("Chadbone.Events");

  test("on and trigger", 2, function() {
    var obj = { counter: 0 };
    _.extend(obj,Chadbone.Events);
  ...
  test("listenTo and stopListening", 1, function() {
    var a = _.extend({}, Chadbone.Events);
    var b = _.extend({}, Chadbone.Events);
  ...
  test("listenTo yourself", 1, function(){
    var e = _.extend({}, Chadbone.Events);
  ...
  test("bind a callback with a supplied context", 1, function () {
    ...
    var obj = _.extend({},Chadbone.Events);
  ...
  test("callback list is not altered during trigger", 2, function () {
    var counter = 0, obj = _.extend({}, Chadbone.Events);
  ...
  test("once variant one", 3, function() {
    ...
    var a = _.extend({}, Chadbone.Events).once('event', f);
    var b = _.extend({}, Chadbone.Events).on('event', f);
  ...
  test("Chadbone object inherits Events", function() {
    ok(Chadbone.on === Chadbone.Events.on);
  });

  asyncTest("once with asynchronous events", 1, function() {
    var func = _.debounce(function() { ok(true); start(); }, 50);
    var obj = _.extend({}, Chadbone.Events).once('async', func);
  ...
  test("`once` on `all` should work as expected", 1, function() {
    Chadbone.once('all', function() {
      ok(true);
      Chadbone.trigger('all');
    });
    Chadbone.trigger('all');
  });

  test("once without a callback is a noop", 0, function() {
    _.extend({}, Chadbone.Events).once('event').trigger('event');
  });

  test("event functions are chainable", function() {
    var obj = _.extend({}, Chadbone.Events);
    var obj2 = _.extend({}, Chadbone.Events);
    var fn = function() {};
