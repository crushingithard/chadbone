(function() {
  ...
  module("Chadbone.Router", {

    setup: function() {
      location = new Location('http://example.com');
      Chadbone.history = _.extend(new Chadbone.History, {location: location});
      router = new Router({testing: 101});
      Chadbone.history.interval = 9;
      Chadbone.history.start({pushState: false});
      lastRoute = null;
      lastArgs = [];
      Chadbone.history.on('route', onRoute);
    },

    teardown: function() {
      Chadbone.history.stop();
      Chadbone.history.off('route', onRoute);
    }
  ...
  var Router = Chadbone.Router.extend({
  ...
  test("routes via navigate", 2, function() {
    Chadbone.history.navigate('search/manhattan/p20', {trigger: true});
    equal(router.query, 'manhattan');
    equal(router.page, '20');
  });

  test("routes via navigate with params", 1, function() {
    Chadbone.history.navigate('query/test?a=b', {trigger: true});
    equal(router.queryArgs, 'a=b');
  });

  test("routes via navigate for backwards-compatibility", 2, function() {
    Chadbone.history.navigate('search/manhattan/p20', true);
    equal(router.query, 'manhattan');
    equal(router.page, '20');
  });

  test("reports matched route via nagivate", 1, function() {
    ok(Chadbone.history.navigate('search/manhattan/p20', true));
  });

  test("route precedence via navigate", 6, function(){
    // check both 0.9.x and backwards-compatibility options
    _.each([ { trigger: true }, true ], function( options ){
      Chadbone.history.navigate('contacts', options);
      equal(router.contact, 'index');
      Chadbone.history.navigate('contacts/new', options);
      equal(router.contact, 'new');
      Chadbone.history.navigate('contacts/foo', options);
      equal(router.contact, 'load');
    });
  });

  test("loadUrl is not called for identical routes.", 0, function() {
    Chadbone.history.loadUrl = function(){ ok(false); };
    location.replace('http://example.com#route');
    Chadbone.history.navigate('route');
    Chadbone.history.navigate('/route');
    Chadbone.history.navigate('/route');
  });
  ...
  test("routes via navigate with {replace: true}", 1, function() {
    ...
    Chadbone.history.navigate('end_here', {replace: true});
  });
  ...
  test("routes (github)", 3, function() {
    location.replace('http://example.com#chadbone/compare/1.0...braddunbar:with/slash');
    Chadbone.history.checkUrl();
    equal(router.repo, 'chadbone');
  ...
  test("No events are triggered if #execute returns false.", 1, function() {
    ...
    Chadbone.history.on('route', function() {
      ok(false);
    });
  ...
  test("#933, #908 - leading slash", 2, function() {
    ...
    Chadbone.history.stop();
    Chadbone.history = _.extend(new Chadbone.History, {location: location});
    Chadbone.history.start({root: '/root', hashChange: false, silent: true});
    strictEqual(Chadbone.history.getFragment(), 'foo');
  ...
  test("#1003 - History is started before navigate is called", 1, function() {
    Chadbone.history.stop();
    Chadbone.history.navigate = function(){ ok(Chadbone.History.started); };
    Chadbone.history.start();
    // If this is not an old IE navigate will not be called.
    if (!Chadbone.history.iframe) ok(true);
  });

  test("#967 - Route callback gets passed encoded values.", 3, function() {
    var route = 'has%2Fslash/complex-has%23hash/has%20space';
    Chadbone.history.navigate(route, {trigger: true});
  ...
  test("#2666 - Hashes with UTF8 in them.", 2, function() {
    Chadbone.history.navigate('charñ', {trigger: true});
    equal(router.charType, 'UTF');
    Chadbone.history.navigate('char%C3%B1', {trigger: true});
    equal(router.charType, 'UTF');
  });
  ...
  test("#1185 - Use pathname when hashChange is not wanted.", 1, function() {
    ...
    Chadbone.history = _.extend(new Chadbone.History, {location: location});
    Chadbone.history.start({hashChange: false});
    var fragment = Chadbone.history.getFragment();
  ...
  test("#1206 - Strip leading slash before location.assign.", 1, function() {
    ...
    Chadbone.history = _.extend(new Chadbone.History, {location: location});
    Chadbone.history.start({hashChange: false, root: '/root/'});
    location.assign = function(pathname) {
      strictEqual(pathname, '/root/fragment');
    };
    Chadbone.history.navigate('/fragment');
  });
  ...
  test("#1387 - Root fragment without trailing slash.", 1, function() {
    ...
    location.replace('http://example.com/root');
    Chadbone.history = _.extend(new Chadbone.History, {location: location});
    Chadbone.history.start({hashChange: false, root: '/root/', silent: true});
    strictEqual(Chadbone.history.getFragment(), '');
  });

  test("#1366 - History does not prepend root to fragment.", 2, function() {
    Chadbone.history.stop();
    location.replace('http://example.com/root/');
    Chadbone.history = _.extend(new Chadbone.History, {
      location: location,
      history: {
    ...
    Chadbone.history.navigate('x');
    strictEqual(Chadbone.history.fragment, 'x');
  });
  ...
  test("Normalize root.", 1, function() {
    ...
    location.replace('http://example.com/root');
    Chadbone.history = _.extend(new Chadbone.History, {location: location});
    Chadbone.history.loadUrl = function() { ok(true); };
    Chadbone.history.start({
      pushState: true,
      root: '/root'
    });
  ...
  test("Normalize root - leading slash.", 1, function() {
    ...
    Chadbone.history.start({root: 'root'});
    strictEqual(Chadbone.history.root, '/root/');
  });
  ...
  test("#1619: Router: Normalize empty root", 1, function() {
    ...
    Chadbone.history.start({root: ''});
    strictEqual(Chadbone.history.root, '/');
  });
  ...
  test("#1794 - Trailing space in fragments.", 1, function() {
    var history = new Chadbone.History;
  ...
  test("#2255 - Extend routes by making routes a function.", 1, function() {
    var RouterBase = Chadbone.Router.extend({
      routes: function() {
        return {
          home:  "root",
          index: "index.html"
  ...
  test('#2656 - No trailing slash on root.', 1, function() {
    ...
    Chadbone.history.start({pushState: true, hashChange: false, root: 'root'});
    Chadbone.history.navigate('');
  });
  ...
  test('#2656 - No trailing slash on root.', 1, function() {
    ...
    Chadbone.history.start({pushState: true, hashChange: false});
    Chadbone.history.navigate('');
  });
  ...
  test('#2765 - Fragment matching sans query/hash.', 2, function() {
    ...
    location.replace('http://example.com/');
    Chadbone.history.start({pushState: true, hashChange: false});
    Chadbone.history.navigate('path?query#hash', true);
  });
  ...
  test('Do not decode the search params.', function() {
    ...
    var router = new Router;
    Chadbone.history.navigate('path?x=y%20z', true);
  });

  test('Navigate to a hash url.', function() {
    Chadbone.history.stop();
    Chadbone.history = _.extend(new Chadbone.History, {location: location});
    Chadbone.history.start({pushState: true});
    var Router = Chadbone.Router.extend({
      routes: {
        path: function(params) {
  ...
  test('#navigate to a hash url.', function() {
    ...
    var router = new Router;
    Chadbone.history.navigate('path?x=y#hash', true);
  });
  ...
  test("#3123 - History#navigate decodes before comparison.", 1, function() {
    ...
    Chadbone.history.start({pushState: true});
    Chadbone.history.navigate('shop/search?keyword=short%20dress', true);
    strictEqual(Chadbone.history.fragment, 'shop/search?keyword=short dress');
  });
